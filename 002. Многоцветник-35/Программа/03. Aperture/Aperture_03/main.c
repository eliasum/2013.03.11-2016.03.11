...
void main( void )
{
  ...
  PORTB = (1<<IN_SS0)|(1<<IN_LE)|(1<<IN_SS1)|(1<<IN_SS2); // 1 на выходе
...
*/
    ...
    if(var==0xAA)                            // если маркер передачи на ППРМ-2
    {       
      for(int i=0; i<68; i++)                // приём 21-й команды по UART
      {
        while (!(UCSR0A & (1<<RXC0)));       // ждем завершение приема байта
        R[i] = UDR0;                          
      }      
      ...
      ClearBit(PORTB, IN_LE);                // разрешение загрузки
      ...
      SPI_Write(R[2]);                       // второй байт пакета данных       
      SPI_Write(R[3]);                       // третий байт пакета данных       
      SPI_Write(R[7]);                       // четвертый байт пакета данных  
      
      SetBit(PORTB, IN_LE);                  // запрет загрузки      
      delay_ms(1);
      ClearBit(PORTB, IN_LE);                // разрешение загрузки      
      ...
      SPI_Write(R[15]);                      // третий байт пакета данных       
      SPI_Write(R[11]);                      // четвертый байт пакета данных      
      ...
      SPI_Write(R[22]);                      // первый байт пакета данных      
      SPI_Write(R[23]);                      // второй байт пакета данных       
      SPI_Write(R[18]);                      // третий байт пакета данных       
      SPI_Write(R[19]);                      // четвертый байт пакета данных   
      ...
      SPI_Write(0x00);                       // первый байт пакета данных      
      SPI_Write(R[35]);                      // второй байт пакета данных       
      SPI_Write(R[27]);                      // третий байт пакета данных       
      SPI_Write(R[31]);                      // четвертый байт пакета данных 
      ...
      SPI_Write(R[63]);                      // первый байт пакета данных      
      SPI_Write(R[65]);                      // второй байт пакета данных       
      SPI_Write(R[66]);                      // третий байт пакета данных       
      SPI_Write(R[67]);                      // четвертый байт пакета данных
      ...
      SPI_Write(R[58]);                      // первый байт пакета данных      
      SPI_Write(R[59]);                      // второй байт пакета данных       
      SPI_Write(R[61]);                      // третий байт пакета данных       
      SPI_Write(R[62]);                      // четвертый байт пакета данных  
      ...
      SPI_Write(R[53]);                      // первый байт пакета данных      
      SPI_Write(R[54]);                      // второй байт пакета данных       
      SPI_Write(R[55]);                      // третий байт пакета данных       
      SPI_Write(R[57]);                      // четвертый байт пакета данных 
      ...
      SPI_Write(R[47]);                      // первый байт пакета данных      
      SPI_Write(R[49]);                      // второй байт пакета данных       
      SPI_Write(R[50]);                      // третий байт пакета данных       
      SPI_Write(R[51]);                      // четвертый байт пакета данных   
      ...
      SPI_Write(R[42]);                      // первый байт пакета данных      
      SPI_Write(R[43]);                      // второй байт пакета данных       
      SPI_Write(R[45]);                      // третий байт пакета данных       
      SPI_Write(R[46]);                      // четвертый байт пакета данных 
      ...
      SPI_Write(R[37]);                      // первый байт пакета данных      
      SPI_Write(R[38]);                      // второй байт пакета данных       
      SPI_Write(R[39]);                      // третий байт пакета данных       
      SPI_Write(R[41]);                      // четвертый байт пакета данных   
      
      SetBit(PORTB, IN_LE);                  // запрет загрузки      
      delay_ms(1);     
    }
    
    if(var==0xBB)                            // если маркер передачи на ППРМ-2
    { 
      for(int i=0; i<4; i++)                 // приём 4-х байтовой команды по UART
      {
        while (!(UCSR0A & (1<<RXC0)));       // ждем завершение приема байта
        StartControl[i] = UDR0;                          
      }      
      ...
      SPI_Write(0x01);                       // байт кода команды
      SPI_Write(0x00);                       // первый байт пакета данных      
      SPI_Write(0x00);                       // второй байт пакета данных       
      SPI_Write(0x00);                       // третий байт пакета данных       
      SPI_Write(StartControl[2]);            // четвертый байт пакета данных  
      
      SetBit(PORTB, IN_LE);                  // запрет загрузки      
      delay_ms(1);        
    }    
    
    if(var==0xCC)                            // если маркер передачи на ППРМ-2
    { 
      for(int i=0; i<4; i++)                 // приём 4-х байтовой команды по UART
      {
        while (!(UCSR0A & (1<<RXC0)));       // ждем завершение приема байта
      ...
      // Режим "Контроль":
      // Команда "Начать работу":
      SPI_Write(0xE1);                       // байт кода команды
      SPI_Write(0x00);                       // первый байт пакета данных      
      SPI_Write(0x00);                       // второй байт пакета данных       
      SPI_Write(0x00);                       // третий байт пакета данных       
      SPI_Write(StartTesting[2]);            // четвертый байт пакета данных  
      
      SetBit(PORTB, IN_LE);                  // запрет загрузки      
      delay_ms(1);        
    } 

    if(var==0xDD)                            // если маркер передачи на ППРМ-2
    { 
      for(int i=0; i<4; i++)                 // приём 4-х байтовой команды по UART
      {
        while (!(UCSR0A & (1<<RXC0)));       // ждем завершение приема байта
      ...
      // передача данных по SPI на ППРМ-2: 
      // Режим "Работа":
      // Команда "Закончить работу":
      SPI_Write(0x02);                       // байт кода команды
      SPI_Write(0x00);                       // первый байт пакета данных      
      SPI_Write(0x00);                       // второй байт пакета данных       
      SPI_Write(0x00);                       // третий байт пакета данных       
      SPI_Write(StopControl[2]);             // четвертый байт пакета данных  
      
      SetBit(PORTB, IN_LE);                  // запрет загрузки      
      delay_ms(1);        
    }    

    if(var==0xEE)                            // если маркер передачи на ППРМ-2
    { 
      for(int i=0; i<4; i++)                 // приём 4-х байтовой команды по UART
      {
        while (!(UCSR0A & (1<<RXC0)));       // ждем завершение приема байта
      ...
      // Режим "Контроль":
      // Команда "Закончить работу":
      SPI_Write(0xE2);                       // байт кода команды
      SPI_Write(0x00);                       // первый байт пакета данных      
      SPI_Write(0x00);                       // второй байт пакета данных       
      SPI_Write(0x00);                       // третий байт пакета данных       
      SPI_Write(StopTesting[2]);             // четвертый байт пакета данных  
...
/* 
  ...
  - DORD = 1 - первым передаётся младший бит,
