...
int Command8[4]={246, 0, 0, 0};
int Command9[4]={247, 0, 0, 0};
...
*/
    ...
    // Команда "Подавление I-го гетеродинного сигнала"
    int Comm8=Command8[3]+Command8[2]*256;
    Ini->WriteString("Данные в режиме Управление","Команда \"Подавление I-го гетеродинного сигнала\"",Comm8);  // запись из массива в файл

    // Команда "Подавление Q-го гетеродинного сигнала"
    int Comm9=Command9[3]+Command9[2]*256;
    Ini->WriteString("Данные в режиме Управление","Команда \"Подавление Q-го гетеродинного сигнала\"",Comm9);  // запись из массива в файл
...
*/
      ...
      // Начальные значения для передачи по com-порту после загрузки
      Command6[3]=Mod[0][0][1];               // мл. байт
      Command6[2]=Mod[0][0][0];               // ст. байт
      Command7[3]=Mod[0][0][2];
      Shift[3]=Mod[0][0][3];
      ...
      // Команда "Подавление I-го гетеродинного сигнала"
      int Comm8=StrToInt(Ini->ReadString("Данные в режиме Управление","Команда \"Подавление I-го гетеродинного сигнала\"","0"));      // считать из файла в массив
      Command8[3]=Comm8&0xFF;                             // мл. байт
      Command8[2]=(Comm8/256)&0xFF;                       // ст. байт
      for(int i=0; i<=256; i++)
      {
        if(Comm8==256-i) CB_Command8->ItemIndex=i;        // запись в форму
      }
      
      for(int i=257; i<=513; i++)
      {
        if(Comm8==65535+257-i) CB_Command8->ItemIndex=i;  // запись в форму
      }

      // Команда "Подавление Q-го гетеродинного сигнала"
      int Comm9=StrToInt(Ini->ReadString("Данные в режиме Управление","Команда \"Подавление Q-го гетеродинного сигнала\"","0"));      // считать из файла в массив
      Command9[3]=Comm9&0xFF;                             // мл. байт
      Command9[2]=(Comm9/256)&0xFF;                       // ст. байт
      for(int i=0; i<=256; i++)
      {
        if(Comm9==256-i) CB_Command9->ItemIndex=i;        // запись в форму
      }
      
      for(int i=257; i<=513; i++)
      {
        if(Comm9==65535+257-i) CB_Command9->ItemIndex=i;  // запись в форму
      }
...
void __fastcall TMain_Form::SendControl(void)
{
  ...
  if(ChB_Command8->Checked==false)
  {
    for(int i=0; i<4; i++)
    {
      CommPort1->PutChar((char)Command8[i]);
    }
    Delay();                                     // задержка
  }

  if(ChB_Command9->Checked==false)
  {
    for(int i=0; i<4; i++)
    {
      CommPort1->PutChar((char)Command9[i]);
    }
    Delay();                                     // задержка
  }
}
...
void __fastcall TMain_Form::SendTerminal()
{
  ...
  if(ChB_Command8->Checked==false)
  {
    CommPort1->PutString("Command8 = ");

    for(int i=0; i<4; i++)
    {
      uart_puthex_byte((char)Command8[i]);
    }
  ...
  if(ChB_Command9->Checked==false)
  {
    CommPort1->PutString("Command9 = ");

    for(int i=0; i<4; i++)
    {
      uart_puthex_byte((char)Command9[i]);
    }
...
//---------------------------------------------------------------------------
void __fastcall TMain_Form::CB_Command8Change(TObject *Sender)
{
  if(CB_Command8->ItemIndex<257)
  {
    for(int i=0; i<257; i++)
    {
      int code;
      if(CB_Command8->ItemIndex==i) code=256-i;   // получение кода параметра
      int code_LSB=code&0xFF;                     // младший байт параметра
      int code_MSB=(code/256)&0xFF;               // старший байт параметра
      Command8[2]=code_MSB;                       // записать значение старшего байта параметра
      Command8[3]=code_LSB;                       // записать значение младшего байта параметра
    }
  }
  ...
  if(CB_Command8->ItemIndex>=257)
  {
    ...
    {
      long int code;
      if(CB_Command8->ItemIndex==i) code=65535+257-i;  // получение кода параметра
      int code_LSB=code&0xFF;                     // младший байт параметра
      int code_MSB=(code/256)&0xFF;               // старший байт параметра
      Command8[2]=code_MSB;                       // записать значение старшего байта параметра
      Command8[3]=code_LSB;                       // записать значение младшего байта параметра
  ...
  //Label7->Caption=IntToStr(Command8[2]);
  //Label10->Caption=IntToStr(Command8[3]);
}
//---------------------------------------------------------------------------
void __fastcall TMain_Form::CB_Command9Change(TObject *Sender)
{
  if(CB_Command9->ItemIndex<257)
  {
    for(int i=0; i<257; i++)
    {
      int code;
      if(CB_Command9->ItemIndex==i) code=256-i;   // получение кода параметра
      int code_LSB=code&0xFF;                     // младший байт параметра
      int code_MSB=(code/256)&0xFF;               // старший байт параметра
      Command9[2]=code_MSB;                       // записать значение старшего байта параметра
      Command9[3]=code_LSB;                       // записать значение младшего байта параметра
    }
  }
  ...
  if(CB_Command9->ItemIndex>=257)
  {
    ...
    {
      long int code;
      if(CB_Command9->ItemIndex==i) code=65535+257-i;  // получение кода параметра
      int code_LSB=code&0xFF;                     // младший байт параметра
      int code_MSB=(code/256)&0xFF;               // старший байт параметра
      Command9[2]=code_MSB;                       // записать значение старшего байта параметра
      Command9[3]=code_LSB;                       // записать значение младшего байта параметра
  ...
  //Label7->Caption=IntToStr(Command9[2]);
  //Label10->Caption=IntToStr(Command9[3]);
}
//---------------------------------------------------------------------------
