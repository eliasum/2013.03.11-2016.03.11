...
void __fastcall TMain_Form::GoNIDAQmx()
{
  ...
  switch(NIDAQmxCounter)
  {
    ...
    case 2:                                    // Ток потребления источника +5V
      ...
      if((value>1e10)||(value<-1e10)) value=0; // при отключениии NI USB-6008 от компьютера ошибки не будет
      value=(value-0.5)/0.185;                 // коэффициент пропорциональности
      min=(0.5-0.5)/0.185;                     // граничные значения тока
      max=(4.5-0.5)/0.185;
    ...
    case 4:                                    // Температура окружающей среды
      ...
      if((value>1e10)||(value<-1e10)) value=0; // при отключениии NI USB-6008 от компьютера ошибки не будет
      value=(value-0.56)*500;                  // коэффициент пропорциональности
      min=(0.45-0.56)*500;                     // граничные значения температуры
      max=(0.75-0.56)*500;
    ...
    case 5:                                    // СВЧ мощность в точке ВСК2
      ...
      if((value>1e10)||(value<-1e10)) value=0; // при отключениии NI USB-6008 от компьютера ошибки не будет
      value=-(value-1)*37.5;                   // коэффициент пропорциональности
      min=-(1.75-1)*37.5;                      // граничные значения мощности
      max=-(0.5-1)*37.5;
...
// Подпрограмма расчета "скользящего среднего"
{
  ...
  if (N_zamera0 == 10)
  {
    AddSaveLog("Напряжение источника +5В составляет "+P_U5V->Caption);  // добавить комментарий и сохранить лог
    N_zamera0 = 0;
  }
...
// Подпрограмма расчета "скользящего среднего"
{
  ...
  if (N_zamera1 == 10)
  {
    AddSaveLog("Напряжение источника +12В составляет "+P_U12V->Caption);  // добавить комментарий и сохранить лог
    N_zamera1 = 0;
  }
...
// Подпрограмма расчета "скользящего среднего"
{
  ...
  if (N_zamera2 == 10)
  {
    AddSaveLog("Ток источника +5В составляет "+P_I5V->Caption);  // добавить комментарий и сохранить лог
    N_zamera2 = 0;
  }
...
// Подпрограмма расчета "скользящего среднего"
{
  ...
  if (N_zamera3 == 10)
  {
    AddSaveLog("Ток источника +12В составляет "+P_I12V->Caption);  // добавить комментарий и сохранить лог
    N_zamera3 = 0;
  }
...
// Подпрограмма расчета "скользящего среднего"
{
  ...
  if (N_zamera4 == 10)
  {
    AddSaveLog("Температура окружающей среды составляет "+P_Temp->Caption);  // добавить комментарий и сохранить лог
    N_zamera4 = 0;
  }
...
// Подпрограмма расчета "скользящего среднего"
{
  ...
  if (N_zamera5 == 10)
  {
    AddSaveLog("СВЧ мощность в точке ВСК2 составляет "+P_Power->Caption);  // добавить комментарий и сохранить лог
    N_zamera5 = 0;
  }
