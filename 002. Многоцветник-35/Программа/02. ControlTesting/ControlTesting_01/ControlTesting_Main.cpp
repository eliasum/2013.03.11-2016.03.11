...
void __fastcall TMain_Form::SB_OnOffClick(TObject *Sender)
{
  if(SB_OnOff->Down)         // если нажата
  {
    ...
    AddSaveLog("Включено");  // добавить комментарий и сохранить лог
  ...
  else                       // если не нажата
  {
    ...
    AddSaveLog("Выключено. Время работы составило "+P_WorkTime->Caption);  // добавить комментарий и сохранить лог
...
{
/*
  Подготовка команды "Амплитуда блестящей точки (БТ)".
...
/*
  Подготовка команды "Время задержки (БТ)".
  size_t - возвращаемое значение длины строки целочисленного типа без знака функции std::string::size.
*/
...
/*
  Подготовка команды "Доплеровский сдвиг частоты (БТ)".
  size_t - возвращаемое значение длины строки целочисленного типа без знака функции std::string::size.
*/
  if((size_t)LV_Sar1->Items->Count!=0) // если количество строк LV_Sar1 не нулевое
  {
...
// по изменении номера БТ
{
  ...
  Command3[OldIndex][3]=StrToInt(E_Command3_2->Text);  // записать соответствующую амплитуду БТ в массив для предыдущей точки
  OldIndex=i;                                          // сохранить текущий порядковый номер
  E_Command3_2->Text=Command3[i][3];                   // показать текущее значение
}
//---------------------------------------------------------------------------
...
// по изменении номера БТ
{
  ...
  Command4[OldIndex][3]=StrToInt(E_Command4_2->Text);  // записать соответствующее время задержки БТ в массив для предыдущей точки
  OldIndex=i;                                          // сохранить текущий порядковый номер
  E_Command4_2->Text=Command4[i][3];                   // показать текущее значение
}
//---------------------------------------------------------------------------
...
// по изменении номера БТ
{
  ...
  Command5[OldIndex][3]=StrToInt(E_Command5_2->Text);  // записать соответствующий допл. сдвиг частоты БТ в массив для предыдущей точки
  OldIndex=i;                                          // сохранить текущий порядковый номер
  E_Command5_2->Text=Command5[i][3];                   // показать текущее значение
}
//---------------------------------------------------------------------------
