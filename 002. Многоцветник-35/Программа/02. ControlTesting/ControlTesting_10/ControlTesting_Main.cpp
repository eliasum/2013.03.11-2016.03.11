...
int Command4[32][4];
int Command5[32][4];
int Command5_05[32][4]; // цена младшего разряда 0.5 Гц
int D_SignC[32];        // знак доплеровского смещения БТ
...
int Com4[32][4];
int Com5[32][4];
int Com5_05[32][4];     // цена младшего разряда 0.5 Гц
int D_SignT[32];        // знак доплеровского смещения БТ
...
*/
  ...
  for(int i=0;i<StrToInt(E_Command3_0->Text);++i)  // перебор БТ
  {
    ...
    double fdi05=K1*K2/0.5;                        // доплеровское смещение БТ [Гц], цена младшего разряда 0.5 Гц
    ...
    int fdi05_LSB=((int)fdi05)&0xFF;               // младший байт, цена младшего разряда 0.5 Гц
    int fdi05_MSB=((int)fdi05/256)&0xFF;           // старший байт, цена младшего разряда 0.5 Гц
    Command5[i][2]=fdi05_MSB;                      // записать значение старшего байта, цена младшего разряда 0.5 Гц
    Command5[i][3]=fdi05_LSB;                      // записать значение младшего байта, цена младшего разряда 0.5 Гц
    Command5_05[i][2]=fdi_MSB;                     // записать значение старшего байта доплеровского смещения БТ
    Command5_05[i][3]=fdi_LSB;                     // записать значение младшего байта доплеровского смещения БТ
    ...
    Com5[i][2]=fdi05_MSB;                          // записать значение старшего байта, цена младшего разряда 0.5 Гц
    Com5[i][3]=fdi05_LSB;                          // записать значение младшего байта, цена младшего разряда 0.5 Гц
  ...
  E_Command5->Text=IntToStr(Command5_05[OldIndex][3]+Command5_05[OldIndex][2]*256); // показать текущее значение
...
void __fastcall TMain_Form::E_Command5Change(TObject *Sender)
{
  ...
  Comm5=StrToFloat(E_Command5->Text);            // записать значение в [гц] в переменную, цена младшего разряда 0.5 Гц
...
void __fastcall TMain_Form::E_Command5Exit(TObject *Sender)
{
  ...
  if(b)
  {
    ...
    float Comm5=StrToFloat(E_Command5->Text);               // записать значение в [гц] в переменную, цена младшего разряда 0.5 Гц 
    Command5[OldIndex][3]=(int)Comm5&0xFF;                  // мл. байт
    Command5[OldIndex][2]=((int)Comm5/256)&0xFF;            // ст. байт
  }
  else
...
void __fastcall TMain_Form::E_Com5Exit(TObject *Sender)
{
  ...
  if(b)
  {
    ...
    int Comm5=StrToInt(E_Com5->Text);               // записать значение в [гц] в переменную, цена младшего разряда 0.5 Гц
...
void __fastcall TMain_Form::E_Com5Change(TObject *Sender)
{
  ...
  Comm5=StrToInt(E_Com5->Text);                    // записать значение в [гц] в переменную, цена младшего разряда 0.5 Гц
  Com5[OldIndex_][3]=(int)Comm5&0xFF;              // мл. байт
  Com5[OldIndex_][2]=((int)Comm5/256)&0xFF;        // ст. байт
}
//---------------------------------------------------------------------------
...
void __fastcall TMain_Form::InitBPControl(void)  // инициализация команд Command 3...5
{
  ...
  for(int i=0; i<32; i++)
  {
    Command3[i][0]=16;  // байт кода команды
    Command3[i][1]=i;   // 1-й байт пакета данных
  ...
  for(int i=0; i<32; i++)
  {
    Command4[i][0]=18;  // байт кода команды
    Command4[i][1]=i;   // 1-й байт пакета данных
  ...
  for(int i=0; i<32; i++)
  {
    Command5[i][0]=17;  // байт кода команды
    Command5[i][1]=i;   // 1-й байт пакета данных
