...
{
  PointIndex=cbLCnBT->ItemIndex;              // записать новый порядковый номер БТ

  //M_PointIndex->Lines->Add(PointIndex);
  //M_ValueIndex->Lines->Add(BPIndexes[PointIndex]);
  ...
  XMax=XString[OldIndex];                     // порядок введения БТ
  YMax=YString[OldIndex];

  OldIndex=PointIndex;                        // сохранить текущий порядковый номер

  // алгоритм подсвечивания выбранной точки
  int Y=0;
  for (int t=0;t<Series1->Count();t++)
  {
    int flag[32];
    if(YString[t]>150) flag[t]=1;
    Y+=flag[t];
    // подсвечивание только после введения всех точек
    if(Y==Series1->Count()) Series1->ValueColor[PointIndex]=clYellow;
  }

  //Series1->ValueColor[PointIndex]=clYellow;

  edLCx->Text=XString[PointIndex];          // показать текущие значения
  edLCy->Text=YString[PointIndex];
  LE_EPR->Text=EPRString[PointIndex];
}
//---------------------------------------------------------------------------
      ...
      TShiftState Shift, int X, int Y)
{
  ...
  if (PtInRect((RECT*)&ChartLC->ChartRect, Point(X-ChartLC->Width3D,Y+ChartLC->Height3D))) {
    ...
    Label1->Caption = Series1->GetHorizAxis->LabelValue(tmpX)
...
// двойной щелчок по графику компонента Chart
{
  ...
  HorizAxis=StrToInt(Series1->GetHorizAxis->LabelValue((int)tmpX));
  VertAxis=StrToInt(Series1->GetVertAxis->LabelValue((int)tmpY));
  
  if((edLCBT->Text!="")&&(cbLCnBT->Text!="")&&(VertAxis>150))
  {
    if((HorizAxis>=XMax)&&(VertAxis>=YMax)) // проверка правильности порядка введения БТ
    {
      int Commx, Commy;
...
void __fastcall TMain_Form::LE_EPRExit(TObject *Sender)
{
  ...
  int max=65535;                                 // максимальное значение
  int delta=1;
...
//---------------------------------------------------------------------------
void __fastcall TMain_Form::ChartLCClickSeries(TCustomChart *Sender,
      TChartSeries *Series, int ValueIndex, TMouseButton Button,
      TShiftState Shift, int X, int Y)
{
  //BPIndexes[PointIndex]=ValueIndex;
