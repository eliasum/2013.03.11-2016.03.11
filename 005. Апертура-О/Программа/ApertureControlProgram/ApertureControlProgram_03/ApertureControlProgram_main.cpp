...
int delay_ms;
enum diapason {D1=1, D2, D31, D32, D4} range;     // диапазоны
...
/*
  if(f>=900&&f<=10000)                        // если диапазон правильный
  {
	...
	if(range == D1)
	{
	  Command6[2]=0;                     // записать значение старшего байта центральной частоты рабочей полосы
...
void __fastcall TMainForm::StartControl(void)
{
  ...
  switch(range)
  {
	...
	case D31:                                 // Д3.1 (7,1-7,9)	1	0	1	0	1
	  SetRange(1, 0, 1, 0, 1);
	  break;

	case D32:                                 // Д3.2 (7,7-8,5) 1	0	1	0	1
	  SetRange(1, 0, 1, 0, 1);
	  break;
