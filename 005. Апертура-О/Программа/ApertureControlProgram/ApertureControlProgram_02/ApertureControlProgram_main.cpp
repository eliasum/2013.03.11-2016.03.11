...
*/
  ...
  DevToP0P1(Dev1P0, Dev1P1);
  SetDevNumber(dv1);
  SetDevP0P1Data(Dev_port0, Dev_port1);

  DevToP0P1(Dev2P0, Dev2P1);
  SetDevNumber(dv2);
  SetDevP0P1Data(Dev_port0, Dev_port1);

  DevToP0P1(Dev3P0, Dev3P1);
  SetDevNumber(dv3);
  SetDevP0P1Data(Dev_port0, Dev_port1);

  DevToP0P1(Dev4P0, Dev4P1);
  SetDevNumber(dv4);
  SetDevP0P1Data(Dev_port0, Dev_port1);

  DevToP0P1(Dev5P0, Dev5P1);
  SetDevNumber(dv5);
  SetDevP0P1Data(Dev_port0, Dev_port1);

  DevToP0P1(Dev6P0, Dev6P1);
  SetDevNumber(dv6);
  SetDevP0P1Data(Dev_port0, Dev_port1);
}
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::B_ReadyClick(TObject *Sender)
{
  ...
  if(f>=900&&f<=10000)                        // если диапазон правильный
  {
	...
	if(f>=7100&&f<=7700) range = D31;
	else
	if(f>=7900&&f<=8500) range = D32;
	else
	if(f>=9200&&f<=1000) range = D4;
...
void __fastcall TMainForm::Button2Click(TObject *Sender)
{
  try
  {
	...
	if(!DigitalLineWrite(1, 1)) throw 2;
	if(!DigitalLineWrite(2, 1)) throw 3;
  }
...
//---------------------------------------------------------------------------
void TMainForm::ClearDevs(void) // очистка портов NI USB-6008
{
  int i;
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::SetDevNumber(int device) // установка номера NI USB-6008
{
  if(device==dv1)
  {
