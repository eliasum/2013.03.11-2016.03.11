...
float Ip27BDminD34=-1;
float Ip27BDmaxD34=1;
...
При нажатии кнопки ВКЛЮЧИТЬ в интерфейсе программы, производится проверка токов
потребления и напряжений всех восьми блоков устройства А-О. Если токи
потребления имеют отклонение от номинального значения, выдается информационное
сообщение «Блок [название блока] не включен или неисправен».
...
/*
В зависимости от выбранного частотного диапазона выставить соответствующие
значения частоты гетеродина в блоке синтезатора (блок БС).
*/
  //SetFreqSPI();  // Установка значения частоты гетеродина в БС
/*
...
// установить канал ПРМ
{
  ...
  try
  {
	if(!SetDevP0P1Data(Dev_port0, Dev_port1)) throw 3;  // запись цифровых портов PRM
  }
...
{
  DevP0[0]=a;                            // 1->DevP0[0]
  SetPRMchan(DevP0, 0);                  // установить канал ПРМ

  DevP0[1]=b;                            // 1->DevP0[1]
  SetPRMchan(DevP0, 1);                  // установить канал ПРМ

  DevP0[2]=c;                            // 1->DevP0[2]
  SetPRMchan(DevP0, 2);                  // установить канал ПРМ

  DevP0[3]=d;                            // 1->DevP0[3]
  SetPRMchan(DevP0, 3);                  // установить канал ПРМ

  DevP0[4]=e;                            // 1->DevP0[4]
  SetPRMchan(DevP0, 4);                  // установить канал ПРМ
}
//---------------------------------------------------------------------------
...
{
  DevP0[0]=a;                            // 1->DevP0[0]
  SetPRDchan(DevP0, 0);                  // установить канал ПРД

  DevP0[1]=b;                            // 1->DevP0[1]
  SetPRDchan(DevP0, 1);                  // установить канал ПРД

  DevP0[2]=c;                            // 1->DevP0[2]
  SetPRDchan(DevP0, 2);                  // установить канал ПРД

  DevP0[3]=d;                            // 1->DevP0[3]
  SetPRDchan(DevP0, 3);                  // установить канал ПРД

  DevP0[4]=e;                            // 1->DevP0[4]
  SetPRDchan(DevP0, 4);                  // установить канал ПРД
}
//---------------------------------------------------------------------------
...
// установить канал ПРД
{
  ...
  try
  {
	if(!SetDevP0P1Data(Dev_port0, Dev_port1)) throw 3;  // запись цифровых портов PRD
  }
...
{
  DevP0[5]=a;                            // 1->DevP0[5]
  SetPRMchan(DevP0, 5);                  // установить канал ПРМ
  SetPRDchan(DevP0, 5);                  // установить канал ПРД

  DevP0[6]=b;                            // 1->DevP0[6]
  SetPRMchan(DevP0, 6);                  // установить канал ПРМ
  SetPRDchan(DevP0, 6);                  // установить канал ПРД

  DevP0[7]=c;                            // 1->DevP0[7]
  SetPRMchan(DevP0, 7);                  // установить канал ПРМ
  SetPRDchan(DevP0, 7);                  // установить канал ПРД

  DevP1[0]=d;                            // 1->DevP1[0]
  SetPRMchan(DevP1, 0);                  // установить канал ПРМ
  SetPRDchan(DevP1, 0);                  // установить канал ПРД

  DevP1[1]=e;                            // 1->DevP1[1]
  SetPRMchan(DevP1, 1);                  // установить канал ПРМ
  SetPRDchan(DevP1, 1);                  // установить канал ПРД
}
//---------------------------------------------------------------------------
...
*/
	...
	case 13:                                   // Напряжение питания источника +5 В [В]
	  value=0;
	  SetDev_ai(VIP);
	  value=GetDevACDData(Dev_ai4);            // Считываем AI4 от NI USB-6008
	...
	case 14:                                   // Ток выносного приёмника [А]
	  value=0;
	  SetDev_ai(VIP);
	  value=GetDevACDData(Dev_ai5);            // Считываем AI5 от NI USB-6008
...
/*
	...
	// УУ
	case 15:                                   // Напряжение питания источника +5 В [В]
	  value=0;
	  SetDev_ai(UU);
	  value=GetDevACDData(Dev_ai0);            // Считываем AI0 от NI USB-6008
	...
	case 16:                                   // Выходное напряжение датчика тока [А]
	  value=0;
	  SetDev_ai(UU);
	  value=GetDevACDData(Dev_ai1);            // Считываем AI1 от NI USB-6008
	...
	case 17:                                   // Напряжение питания источника +12 В [В]
	  value=0;
	  SetDev_ai(UU);
	  value=GetDevACDData(Dev_ai4);            // Считываем AI4 от NI USB-6008
	...
	// БС
	case 18:                                   // Датчик температуры основного канала [град.]
	  value=0;
	  SetDev_ai(BS);
	  value=GetDevACDData(Dev_ai0);            // Считываем AI0 от NI USB-6008
	...
	case 19:                                   // Датчик температуры резервного канала [град.]
	  value=0;
	  SetDev_ai(BS);
	  value=GetDevACDData(Dev_ai1);            // Считываем AI1 от NI USB-6008
	...
	case 20:                                   // Напряжение питания -5В [В]
	  value=0;
	  SetDev_ai(BS);
	  value=GetDevACDData(Dev_ai2);            // Считываем AI2 от NI USB-6008
	...
	case 21:                                   // Ток потребления по +27В [А]
	  value=0;
	  SetDev_ai(BS);
	...
	case 22:                                   // Мощность сигнала контрольного выхода основного канала [Вт]
	  value=0;
	  SetDev_ai(BS);
	  value=GetDevACDData(Dev_ai4);            // Считываем AI4 от NI USB-6008
	  ...
	  P_Po->Caption=FloatToStrF(value,ffFixed,5,2)+" Вт"; // 2 десятичных знака после запятой
	...
	case 23:                                   // Мощность сигнала контрольного выхода резервного канала [Вт]
	  value=0;
	  SetDev_ai(BS);
	  value=GetDevACDData(Dev_ai5);            // Считываем AI5 от NI USB-6008
	  ...
	  P_Pr->Caption=FloatToStrF(value,ffFixed,5,2)+" Вт"; // 2 десятичных знака после запятой
	...
	case 24:                                   // Напряжение питания +5В [В]
	  value=0;
	  SetDev_ai(BS);
	...
	// ФРЛП
	case 25:                                   // Напряжение питания УЦПС [В], при вкл. блока - 5 В (+4,85…+5,15 В)
	  value=0;
	  SetDev_ai(FRLP);
	  value=GetDevACDData(Dev_ai0);            // Считываем AI0 от NI USB-6008
	...
	case 26:                                   // Датчик температуры [град.]
	  value=0;
	  SetDev_ai(FRLP);
	  value=GetDevACDData(Dev_ai1);            // Считываем AI1 от NI USB-6008
	...
	case 27:                                   // Ток потребления +27 В блока ФРЛП [А]
	  value=0;
	  SetDev_ai(FRLP);
	  value=GetDevACDData(Dev_ai4);            // Считываем AI4 от NI USB-6008
	...
	case 28:                                   // Напряжение аналогового выхода детектора [дБм]
	  value=0;
	  SetDev_ai(FRLP);
	  value=GetDevACDData(Dev_ai5);            // Считываем AI5 от NI USB-6008
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::SetFreqSPI(void)  // Установка значения частоты гетеродина в БС
{
/*
  1) Перед началом измерения напряжений питания необходимо включить питание
  синтезаторов частоты основного и резервного каналов. Для этого на выводах P0.1
...
*/
  ...
  SPIWrite();  // запись значения частоты гетеродина в блок БС по SPI
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::SPISend(char* SendData, char addr, char data)
{
