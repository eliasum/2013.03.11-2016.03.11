...
/*
  ...
  AI0 - чтение отрицательного источника питания -6В канала №1
  AI1 - чтение положительного источника питания +6В канала №1
  AI2 - чтение положительного источника питания +20В канала №1

  Устройство 2:
  AI0 - чтение отрицательного источника питания -6В канала №2
  AI1 - чтение положительного источника питания +6В канала №2
  AI2 - чтение положительного источника питания +20В канала №2

  Устройство 3:
  AI0 - чтение отрицательного источника питания -6В канала №3
  AI1 - чтение положительного источника питания +6В канала №3
  AI2 - чтение положительного источника питания +20В канала №3

  Устройство 4:
  AI0 - чтение отрицательного источника питания -6В канала №4
  AI1 - чтение положительного источника питания +6В канала №4
  AI2 - чтение положительного источника питания +20В канала №4
*/
...
float FNominal1[8]; // номинальные значения токов и
float FNominal2[8]; // напряжений источников питания блоков ИПИ,
float FNominal3[8]; // а так же время паузы и работы каналов
float FNominal4[8];
      ...
      if(RB_Selected1->Checked==true)
      {
        ...
        for(int i=0; i<8; i++)
        {
          ...
          else
          {
            ...
            Application->MessageBox("Файл испытаний стенда Одноцветников в канал 1 загружен!","Внимание",MB_ICONWARNING);
            FNominal1[i]=StrToFloat(Nominal1[i]);
            break;
          }
        }
        /*
      ...
      if(RB_Selected2->Checked==true)
      {
        ...
        for(int i=0; i<8; i++)
        {
          ...
          else
          {
            ...
            Application->MessageBox("Файл испытаний стенда Одноцветников в канал 2 загружен!","Внимание",MB_ICONWARNING);
            FNominal2[i]=StrToFloat(Nominal2[i]);
            break;
          }
        }
        /*
      ...
      if(RB_Selected3->Checked==true)
      {
        ...
        for(int i=0; i<8; i++)
        {
          ...
          else
          {
            ...
            Application->MessageBox("Файл испытаний стенда Одноцветников в канал 3 загружен!","Внимание",MB_ICONWARNING);
            FNominal3[i]=StrToFloat(Nominal3[i]);
            break;
          }
        }
        /*
      ...
      if(RB_Selected4->Checked==true)
      {
        ...
        for(int i=0; i<8; i++)
        {
          ...
          else
          {
            ...
            Application->MessageBox("Файл испытаний стенда Одноцветников в канал 4 загружен!","Внимание",MB_ICONWARNING);
            FNominal4[i]=StrToFloat(Nominal4[i]);
            break;
          }
        }
        /*
...
{
  // Ток ИПО1:
  Source[0] = ReadAnalogChan(Dev1_ai0);     // чтение аналогового канала 0 порта AI устройства 1
  //Source[0] = MovingAverage0(Source[0]);  // расчет "скользящего среднего"
  LE_I_IPO1->Text=RoundValue(0.01, FloatToStr(Source[0])); // 2 десятичных знака после запятой

 // NotEmptyString(Nominal1[0]);
/*
  float minNominal0, maxNominal0;
  minNominal0=StrToFloat(Nominal1[0])*0.7;
  maxNominal0=StrToFloat(Nominal1[0])*1.3;
*/
  float minNominal0, maxNominal0;
  minNominal0=FNominal1[0]*0.7;
  maxNominal0=FNominal1[0]*1.3;

  Memo1->Lines->Add(FNominal1[0]);
  //Memo1->Lines->Add(maxNominal0);
  
  if(Source[0]>maxNominal0&&Source[0]<minNominal0)  // проверка Тока ИПО1 -6В
  {
    T_Checkm6V->Enabled=false;
    Application->MessageBox("Отказ отрицательного источника питания ИПО1!","Внимание",MB_ICONWARNING);
    B_Go->Enabled=true;
    B_Stop->Enabled=false;
