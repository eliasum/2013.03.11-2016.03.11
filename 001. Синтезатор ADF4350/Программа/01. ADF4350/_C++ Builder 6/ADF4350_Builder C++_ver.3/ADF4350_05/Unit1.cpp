  ...
  : TForm(Owner)
{
  ...
  Reg[2]=0x00004EC2;
  Reg[3]=0x000004B3;
  Reg[4]=0x00A50124; // было 0x00A5003C
  Reg[5]=0x00580005;
...
{
  CommPort1->Open=false;             // закрыть com-порт
  Timer2->Enabled=false;             // отключить Timer2
  RadioButton6->Enabled=true;        // разрешить Alternate
  RadioButton7->Enabled=true;        // разрешить Sweep
  CommPort1->BaudRate=cbr2400;       // скорость обмена 2400...
  CB_BaudRate->ItemIndex=0;          // ...выбрать в CB_BaudRate
  CB_BaudRate->Enabled=true;         // разешить CB_BaudRate
  SendDirect->Caption="Send direction: to Synthesizer";
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::RB_Parallel_linesClick(TObject *Sender)
{
  CommPort1->Open=false;             // закрыть com-порт
  Timer2->Enabled=false;             // отключить Timer2
  RadioButton5->Checked=true;        // выбрать Stop
  RadioButton6->Enabled=false;       // запретить Alternate
  RadioButton7->Enabled=false;       // запретить Sweep
  CommPort1->BaudRate=cbr2400;       // скорость обмена 2400...
  CB_BaudRate->ItemIndex=0;          // ...выбрать в CB_BaudRate
  CB_BaudRate->Enabled=false;        // запретить CB_BaudRate
  SendDirect->Caption="Send direction: to Parallel lines";
}
//---------------------------------------------------------------------------
