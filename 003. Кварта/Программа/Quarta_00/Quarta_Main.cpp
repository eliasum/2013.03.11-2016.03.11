...
int Com5_05[32][4];              // цена младшего разряда 0.59 Гц
...
*/
  ...
  for(int i=0;i<StrToInt(E_Com3_0->Text);++i)  // перебор БТ
  {
    ...
    //double fdi=2*f*1000000*V*Yi/c*D;             // формула доплеровского смещения БТ [Гц]
    double K1=2*(f/1000)*V*Yi/D;
    double K2=1000000*1000/c;
    double fdi=K1*K2;                              // доплеровское смещение БТ [Гц] для вывода в программе
    double fdi05=K1*K2/0.59;                       // доплеровское смещение БТ [Гц], цена младшего разряда 0.59 Гц
    ...
    int fdi05_LSB=((int)fdi05)&0xFF;               // младший байт, цена младшего разряда 0.59 Гц
    int fdi05_MSB=((int)fdi05/256)&0xFF;           // старший байт, цена младшего разряда 0.59 Гц
    ...
    Com5[i][2]=fdi05_MSB;                          // записать значение старшего байта, цена младшего разряда 0.59 Гц
    Com5[i][3]=fdi05_LSB;                          // записать значение младшего байта, цена младшего разряда 0.59 Гц
    Com5_05[i][2]=fdi_MSB;                         // записать значение старшего байта доплеровского смещения БТ
    Com5_05[i][3]=fdi_LSB;                         // записать значение младшего байта доплеровского смещения БТ
  }

  CB_Com3_1->ItemIndex=OldIndex;                   // номер БТ по умолчанию при изменении числа БТ
  E_Com3->Text=IntToStr(Com3[OldIndex][3]);                             // показать текущее значение
  E_Com4->Text=IntToStr(Com4[OldIndex][3]+Com4[OldIndex][2]*256);       // показать текущее значение
  E_Com5->Text=IntToStr(Com5_05[OldIndex][3]+Com5_05[OldIndex][2]*256); // показать текущее значение
/*
...
void __fastcall TMain_Form::E_Com4Exit(TObject *Sender)
{
  ...
  if(b)
  {
    ...
    float Comm4=StrToInt(E_Com4->Text)/6.66;               // записать значение в переменную
    Com4[OldIndex_][3]=(int)Comm4&0xFF;                   // мл. байт
    Com4[OldIndex_][2]=((int)Comm4/256)&0xFF;             // ст. байт
  }
  else
  {
    E_Com4->SetFocus();
...
void __fastcall TMain_Form::E_Com4Change(TObject *Sender)
{
  ...
  Comm4=StrToInt(E_Com4->Text)/6.66;                   // записать значение в переменную
  Com4[OldIndex_][3]=(int)Comm4&0xFF;                  // мл. байт
  Com4[OldIndex_][2]=((int)Comm4/256)&0xFF;            // ст. байт
}
//---------------------------------------------------------------------------
void __fastcall TMain_Form::E_Com5Exit(TObject *Sender)
{
  ...
  if(b)
  {
    ...
    float Comm5=StrToInt(E_Com5->Text)/0.59;             // записать значение в [гц] в переменную
...
void __fastcall TMain_Form::E_Com5Change(TObject *Sender)
{
  ...
  Comm5=StrToInt(E_Com5->Text)/0.59;              // записать значение в [гц] в переменную
  Com5[OldIndex_][3]=(int)Comm5&0xFF;             // мл. байт
  Com5[OldIndex_][2]=((int)Comm5/256)&0xFF;       // ст. байт
}
//---------------------------------------------------------------------------
...
*/
      ...
      for(int i=0; i<StrToInt(E_Com3_0->Text); i++)
      {
        ...
        // Com4
        int Comm4=6.66*StrToInt(Ini->ReadString("Данные","Команда \"Время задержки (БТ)"+IntToStr(i+1)+"\"","0"));             // считать из файла в массив
        Com4[i][3]=Comm4&0xFF;                // мл. байт
        Com4[i][2]=(Comm4/256)&0xFF;          // ст. байт
        // Com5, цена младшего разряда 0.59 Гц
        int Comm5=0.59*StrToInt(Ini->ReadString("Данные","Команда \"Доплеровский сдвиг частоты (БТ)\""+IntToStr(i+1)+"\"","0")); // считать из файла в массив
...
void __fastcall TMain_Form::SendTesting()
{
  ...
  if(ChB_Com3->Checked==false)
  {
    for(int i=0; i<32; i++)
    {
      ...
      DelayTimer->Enabled=true;              // задержка 1000 мкс
...
//---------------------------------------------------------------------------
void __fastcall TMain_Form::DelayTimerTimer(TObject *Sender)
{
