...
int Com3[32][4];                 // Амплитуда БТ
int Com4[32][4];                 // Задержка БТ
int Com5[32][4];                 // Доплеровское смещение БТ
int D_Sign[32];                  // знак доплеровского смещения БТ

int Com6[4]={19, 0, 0, 0};       // Ослабление цифрового аттенюатора
int Com7[4]={20, 0, 0, 0};       // Сумма амплитуд БТ
int Com8[4]={21, 0, 7, 142};     // Частота среза фильтра гетеродина приемника
int Com9[4]={22, 0, 7, 142};     // Частота среза фильтра гетеродина передатчика
...
// Переменные
int OldIndex_;                  // для режима Управление
...
*/
  ...
  // Com3, Com4, Com5
  for(int i=0;i<StrToInt(E_Com3_0->Text);++i)      // перебор БТ
  {
    ...
    //Tau=((2*Xi)/c-1мкс)*10^9 [нс]                // формула времени задержки БТ [нс]
    //float Tau=2*Xi*3.33-1000;                      // время задержки БТ [нс], цена младшего разряда 1 нс
    float Tau=2*Yi*3.33-1000;                      // время задержки БТ [нс], цена младшего разряда 1 нс
    ...
    Com4[i][3]=Tau_MSB;                            // записать значение старшего байта времени задержки БТ
    Com4[i][2]=Tau_LSB;                            // записать значение младшего байта времени задержки БТ
    // Com5 "Доплеровский сдвиг частоты (БТ)" режима Тестирование
    Com5[i][1]=i+1;                                // записать значение номера БТ
    Com5[i][3]=fdi_MSB;                            // записать значение старшего байта доплеровского смещения БТ
    Com5[i][2]=fdi_LSB;                            // записать значение младшего байта доплеровского смещения БТ
  ...
  E_Com4->Text=IntToStr(Com4[OldIndex_][2]+Com4[OldIndex_][3]*256);  // показать текущее значение
  E_Com5->Text=IntToStr(Com5[OldIndex_][2]+Com5[OldIndex_][3]*256);  // показать текущее значение
...
// по изменении номера БТ
{
  ...
  Com4[OldIndex_][2]=Comm4&0xFF;                     // мл. байт
  Com4[OldIndex_][3]=(Comm4/256)&0xFF;               // ст. байт
  Com5[OldIndex_][2]=Comm5&0xFF;                     // мл. байт
  Com5[OldIndex_][3]=(Comm5/256)&0xFF;                // ст. байт
  D_Sign[OldIndex_]=CB_Sign->ItemIndex;

  OldIndex_=i;
  E_Com3->Text=IntToStr(Com3[i][3]);                 // показать текущее значение
  E_Com4->Text=IntToStr(Com4[i][3]*256+Com4[i][2]);   // показать текущее значение
  E_Com5->Text=IntToStr(Com5[i][3]*256+Com5[i][2]);   // показать текущее значение
...
void __fastcall TMain_Form::CB_Com8Change(TObject *Sender)
{
  for(int i=0; i<16; i++)
  {
    ...
    int code_LSB=code&0xFF;                     // младший байт параметра
    int code_MSB=(code/256)&0xFF;               // старший байт параметра
    Com8[3]=code_MSB;                           // записать значение старшего байта параметра
    Com8[2]=code_LSB;                           // записать значение младшего байта параметра
  }
}
//---------------------------------------------------------------------------
void __fastcall TMain_Form::CB_Com9Change(TObject *Sender)
{
  for(int i=0; i<16; i++)
  {
    ...
    int code_LSB=code&0xFF;                     // младший байт параметра
    int code_MSB=(code/256)&0xFF;               // старший байт параметра
    Com9[3]=code_MSB;                           // записать значение старшего байта параметра
    Com9[2]=code_LSB;                           // записать значение младшего байта параметра
  }
}
//---------------------------------------------------------------------------
...
void __fastcall TMain_Form::ComPortTransmitter()
{
  ...
  else                                           // передача на Устройство, BaudRate=57600
  {
    SendTesting();
  }
...
void __fastcall TMain_Form::E_Com4Exit(TObject *Sender)
{
  ...
  if(b)
  {
    ...
    float Comm4=StrToFloat(E_Com4->Text);                 // записать значение в переменную
    Com4[OldIndex_][2]=(int)Comm4&0xFF;                   // мл. байт
    Com4[OldIndex_][3]=((int)Comm4/256)&0xFF;             // ст. байт
  }
  else
  {
    E_Com4->SetFocus();
...
void __fastcall TMain_Form::E_Com4Change(TObject *Sender)
{
  ...
  Comm4=StrToFloat(E_Com4->Text);                      // записать значение в переменную
  Com4[OldIndex_][2]=(int)Comm4&0xFF;                  // мл. байт
  Com4[OldIndex_][3]=((int)Comm4/256)&0xFF;            // ст. байт
}
//---------------------------------------------------------------------------
void __fastcall TMain_Form::E_Com5Exit(TObject *Sender)
{
  ...
  if(b)
  {
    ...
    int Comm5=StrToFloat(E_Com5->Text);                // записать значение в [гц] в переменную
    Com5[OldIndex_][2]=(int)Comm5&0xFF;                // мл. байт
    Com5[OldIndex_][3]=((int)Comm5/256)&0xFF;          // ст. байт
  }
  else
  {
    E_Com5->SetFocus();
...
void __fastcall TMain_Form::E_Com5Change(TObject *Sender)
{
  ...
  Comm5=StrToFloat(E_Com5->Text);                 // записать значение в [гц] в переменную
  Com5[OldIndex_][2]=(int)Comm5&0xFF;             // мл. байт
  Com5[OldIndex_][3]=((int)Comm5/256)&0xFF;       // ст. байт
}
//---------------------------------------------------------------------------
...
*/
    ...
    Ini->WriteString("Данные","Количество БТ",E_Com3_0->Text);
    // Com3
    for(int i=0; i<StrToInt(E_Com3_0->Text); i++)
    {
      Ini->WriteString("Данные","Команда \"Амплитуда блестящей точки (БТ)\""+IntToStr(i+1)+"\"",Com3[i][3]);  // запись из массива в файл
    }
    // Com4, цена младшего разряда 6.66 нс
    for(int i=0; i<StrToInt(E_Com3_0->Text); i++)
    {
      int Command=(Com4[i][2]+Com4[i][3]*256);
      Ini->WriteString("Данные","Команда \"Время задержки (БТ)"+IntToStr(i+1)+"\"",Command);   // запись из массива в файл
    }
    // Com5, цена младшего разряда 0.59 Гц
    for(int i=0; i<StrToInt(E_Com3_0->Text); i++)
    {
      Ini->WriteString("Данные","Команда \"Знак доплеровского сдвига частоты (БТ)\""+IntToStr(i+1)+"\"",D_Sign[i]);  // запись из массива в файл
      int Command=(Com5[i][2]+Com5[i][3]*256);
      Ini->WriteString("Данные","Команда \"Доплеровский сдвиг частоты (БТ)\""+IntToStr(i+1)+"\"",Command);  // запись из массива в файл
    }
    // Com6
    Ini->WriteString("Данные","Команда \"Ослабление цифрового аттенюатора\"",IntToStr(Com6[3]));   // запись из массива в файл
    // Com7
    Ini->WriteString("Данные","Команда \"Сумма амплитуд БТ\"",IntToStr(Com7[3])); // запись из массива в файл
    // Com8
    int Comm8=Com8[2]+Com8[3]*256;
    Ini->WriteString("Данные","Команда \"Частота среза фильтра гетеродина приёмника\"",Comm8);  // запись из массива в файл
    // Com9
    int Comm9=Com9[2]+Com9[3]*256;
    Ini->WriteString("Данные","Команда \"Частота среза фильтра гетеродина передатчика\"",Comm9);  // запись из массива в файл
    // Com13
...
void __fastcall TMain_Form::TB_LoadTestingClick(TObject *Sender)
{
  ...
  OpenDialog->Filter="Файл режима Управление (*.tst)|*.tst";
...
*/
      // Количество БТ
      E_Com3_0->Text=Ini->ReadString("Данные","Количество БТ","1");
      // Com3, Com4, Com5 
      for(int i=0; i<StrToInt(E_Com3_0->Text); i++)
      {
        // Com3
        Com3[i][3]=StrToInt(Ini->ReadString("Данные","Команда \"Амплитуда блестящей точки (БТ)\""+IntToStr(i+1)+"\"","0")); // считать из файла в массив
        // Com4, цена младшего разряда 6.66 нс
        int Comm4=StrToFloat(Ini->ReadString("Данные","Команда \"Время задержки (БТ)"+IntToStr(i+1)+"\"","0"));             // считать из файла в массив
        Com4[i][2]=Comm4&0xFF;                // мл. байт
        Com4[i][3]=(Comm4/256)&0xFF;          // ст. байт
        // Com5, цена младшего разряда 0.59 Гц
        D_Sign[i]=StrToInt(Ini->ReadString("Данные","Команда \"Знак доплеровского сдвига частоты (БТ)\""+IntToStr(i+1)+"\"","0")); // считать из файла
        if(D_Sign[i]==0) CB_Sign->ItemIndex=0;
        else             CB_Sign->ItemIndex=1;

        int Comm5=StrToInt(Ini->ReadString("Данные","Команда \"Доплеровский сдвиг частоты (БТ)\""+IntToStr(i+1)+"\"","0")); // считать из файла в массив
        Com5[i][2]=Comm5&0xFF;                // мл. байт
        Com5[i][3]=(Comm5/256)&0xFF;          // ст. байт
      }
      CB_Com3_1->Clear();                     // очистить предыдущее количество БТ
      ...
      CB_Com3_1->ItemIndex=0;         // номер БТ по умолчанию при изменении числа БТ  OldIndex_
      if(D_Sign[0]==0) CB_Sign->ItemIndex=0;
      else             CB_Sign->ItemIndex=1;
      E_Com3->Text=Com3[0][3];                 // показать текущее значение
      E_Com4->Text=Com4[0][2]+Com4[0][3]*256;  // показать текущее значение
      E_Com5->Text=Com5[0][2]+Com5[0][3]*256;  // показать текущее значение
      // Com6
      Com6[3]=StrToInt(Ini->ReadString("Данные","Команда \"Ослабление цифрового аттенюатора\"","0")); // считать из файла в массив
      for(int i=0; i<16; i++)
      {
        if(Com6[3]==i) CB_Com6->ItemIndex=i; // запись в форму
      }
      // Com7
      Com7[3]=StrToInt(Ini->ReadString("Данные","Команда \"Сумма амплитуд БТ\"","0")); // считать из файла в массив
      for(int i=0; i<32; i++)
      {
        if(Com7[3]==i) CB_Com7->ItemIndex=i; // запись в форму
      }
      // Com8
      ...
      Com8[2]=Comm8&0xFF;                       // мл. байт
      Com8[3]=(Comm8/256)&0xFF;                 // ст. байт
      for(int i=0; i<=15; i++)
      {
      ...
      Com9[2]=Comm9&0xFF;                       // мл. байт
      Com9[3]=(Comm9/256)&0xFF;                 // ст. байт
      for(int i=0; i<=15; i++)
      {
...
{
  AddSaveLog("Выключено измерение данных. Время работы составило "+P_WorkTime->Caption);  // добавить комментарий и сохранить лог
...
void __fastcall TMain_Form::CB_BaudRateChange(TObject *Sender)
{
  switch(CB_BaudRate->ItemIndex)
  {
    ...
    L_SendDirect->Caption="Напр-е передачи: устройство";
    break;
  }
}
/*************************************************************************
...
{
  AddSaveLog("Передать команды");            // добавить комментарий и сохранить лог

  CommPort1->PutChar((char)0x00);            // байт
  CommPort1->PutChar((char)0x00);            // байт
  CommPort1->PutChar((char)0x00);            // байт
  CommPort1->PutChar((char)0x00);            // байт
  ...
  if(ChB_Com3->Checked==false)
  {
    for(int i=0; i<32; i++)
    {
      ...
      int Comm5=(Com5[i][2]+Com5[i][3]*256)/0.59; // цена младшего разряда 0.59 гц
      int Comm5_LSB[32], Comm5_MSB[32];

      M_Debug->Lines->Add(Comm5);
      M_Debug->Lines->Add(D_Sign[i]);
      ...
      Comm5_MSB[i]=(Comm5/256)&0xFF;         // ст. байт

      M_Debug->Lines->Add(Comm5_LSB[i]);
      M_Debug->Lines->Add(Comm5_MSB[i]);

      CommPort1->PutChar((char)Com5[i][0]);  // байт кода команды
      CommPort1->PutChar((char)Com5[i][1]);  // 1-й байт пакета данных
      CommPort1->PutChar((char)Comm5_LSB[i]);// 2-й байт пакета данных
      CommPort1->PutChar((char)Comm5_MSB[i]);// 3-й байт пакета данных

      Delay();                               // задержка

      int Comm4=(Com4[i][2]+Com4[i][3]*256)/6.66; // цена младшего разряда 6.66 нс
      int Comm4_LSB[32], Comm4_MSB[32];

      Comm4_LSB[i]=Comm4&0xFF;               // мл. байт
      Comm4_MSB[i]=(Comm4/256)&0xFF;         // ст. байт

      CommPort1->PutChar((char)Com4[i][0]);  // байт кода команды
      CommPort1->PutChar((char)Com4[i][1]);  // 1-й байт пакета данных
      CommPort1->PutChar((char)Comm4_LSB[i]);// 2-й байт пакета данных
      CommPort1->PutChar((char)Comm4_MSB[i]);// 3-й байт пакета данных
...
/*
  Порядок передачи команд: код команды, первый байт данных,
  второй байт данных, третий байт данных.
*/
...
void __fastcall TMain_Form::SendTerminal()
{
  ...
  if(ChB_Com3->Checked==false)
  {
    ...
    for(int i=0; i<32; i++)
    {
      ...
      uart_puti(i);                            // передача номера БТ
      CommPort1->PutString(" = ");
...
void __fastcall TMain_Form::InitBPTesting(void)  // инициализация команд Com 3...5
{
  ...
  for(int i=0; i<32; i++)
  {
    Com3[i][0]=16;  // байт кода команды
    Com3[i][1]=i;   // 1-й байт пакета данных
  ...
  for(int i=0; i<32; i++)
  {
    Com4[i][0]=18;  // байт кода команды
    Com4[i][1]=i;   // 1-й байт пакета данных
  ...
  for(int i=0; i<32; i++)
  {
    Com5[i][0]=17;  // байт кода команды
    Com5[i][1]=i;   // 1-й байт пакета данных
    Com5[i][2]=0;   // 2-й байт пакета данных
    Com5[i][3]=0;   // 3-й байт пакета данных
  }

  // инициализация массива Sign
  for(int i=0; i<32; i++)
  {
    D_Sign[i]=0;
  }
